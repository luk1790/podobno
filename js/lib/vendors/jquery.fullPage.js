	...
	$.fn.fullpage = function(options) {
		...
		*/
		function touchMoveHandler(event){
			...
			if (!checkParentForNormalScrollElement(event.target)) {
				...
				if (!isMoving && !slideMoving) { //if theres any #
					...
					$('.end').remove();
					$('body').append('<div class="end" style="z-index:33; position:fixed; top: 40px; font-size:3em; right: 20px;">Move ==> ori: '+touchEvents['all']['originalPageY']+' vs touches: '+touchEvents['all']['touchesPageY']+' - ' +  touchEvents['all']['msPointerEnabled'] + '-' + touchEvents['y'] + '</div>');
		...
		function touchStartHandler(event){
			...
			$('.start').remove();
			$('body').append('<div class="start" style="z-index:33; position:fixed; top: 160px; font-size:3em; right: 20px;">Start ==> ori: '+touchEvents['all']['originalPageY']+' vs touches: '+touchEvents['all']['touchesPageY']+' - ' +  touchEvents['all']['msPointerEnabled'] + '-' +touchEvents['y'] + '</div>');
		}
		...
		*/
		function getEventsPage(e){
			...
			events['y'] = (typeof e.pageY !== 'undefined' && (e.pageY!=0 || e.pageX) ? e.pageY : e.touches[0].pageY);
			events['x'] = (typeof e.pageX !== 'undefined' && (e.pageY || e.pageX) ? e.pageX : e.touches[0].pageX);



			events['all'] = {
				'originalPageY': (typeof e.pageY !== 'undefined'? e.pageY : 'noHay'),
				'touchesPageY': (typeof e.touches[0].pageY !== 'undefined' ? e.touches[0].pageY: 'noHay'),
				'msPointerEnabled': (window.navigator.msPointerEnabled? 'Yes': 'No')
			};
